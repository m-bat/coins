...

{
    ...
    public static Element findContainingElement(Subgraph subg, Point2D pt) {
    	...
    	if(bb.contains(pt)) {
    		...
    		while(enm.hasNext()) {
    			if((subelem = findContainingElement((Subgraph)(enm.next()), pt)) != null) {
    				elem = subelem;
    			}
    		}
    ...
    public static List findContainedElements(Subgraph subg, GrappaBox box) {
    	...
    	if(box.contains(bb)) {
    		return subg.listOfElements(Grappa.SUBGRAPH|Grappa.NODE|Grappa.EDGE);
    	} else if(box.intersects(bb)) {
    		List<Element> subelems = null;
    		Iterator<Subgraph> enm = subg.subgraphElements();
    		while(enm.hasNext()) {
    			if((subelems = findContainedElements((Subgraph)(enm.next()), box)) != null) {
    				if(elems == null) {
    					elems = new ArrayList();
    				}
    				elems.add(subelems);
    ...
    public static void setHighlight(Element elem, int mode, int setting) {
    	...
    	if(mode == 0) {
    		...
    		if(wasDelete) {
    			if(elem.isNode()) {
    				Iterator enm = ((Node)elem).edgeElements();
    				while(enm.hasNext()) {
    					((Element)(enm.next())).highlight = 0;
    				}
    			} else if(elem.isSubgraph()) {
    				Iterator enm = ((Subgraph)elem).elements();
    				while(enm.hasNext()) {
    					((Element)(enm.next())).highlight = 0;
    				}
    			}
    		...
    		if((mode&DELETION_MASK) == DELETION_MASK) {
    			if(elem.isNode()) {
    				if((elem.highlight&DELETION_MASK) == DELETION_MASK) {
    					Iterator enm = ((Node)elem).edgeElements();
    					while(enm.hasNext()) {
    						((Element)(enm.next())).highlight |= DELETION_MASK;
    					}
    				} else {
    					Iterator enm = ((Node)elem).edgeElements();
    					while(enm.hasNext()) {
