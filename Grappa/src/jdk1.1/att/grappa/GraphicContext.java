...
public class GraphicContext implements Cloneable
{
  ...
  // a cache for fonts
  private static Hashtable fontcache = new Hashtable(12,6);

  // used with canonColor
   ...
   */
  public static final Font   defaultFont       = fontFromCache(defaultFontname,defaultFontstyle,defaultFontsize);
  ...
  private int         lineStyle       = Grappa.LINE_SOLID;
  ...
  public static Color getColor(String name, Color color) {
	...
	retColor = new Color(Color.HSBtoRGB(hsb[0],hsb[1],hsb[2]));
	if(retColor == null) {
	  retColor = color;
	} else {
  ...
  /**
   * Create a <code>GraphicContext</code> instance initialized with the
   * default values.
   */
  ...
  public Font setFont() {
    ...
    font = fontFromCache(fontname,fontstyle,fontsize);
  ...
  static Font fontFromCache(String name, int style, int size) {
    Integer hasher = new Integer(name.hashCode() ^ style ^ size);
    Font font = null;
    if((font = (Font)fontcache.get(hasher)) == null) {
      font = new Font(name,style,size);
      fontcache.put(hasher,font);
    }
    return font;
  }
   ...
   */
  public Rectangle setClipRect(int x, int y, int width, int height) {
    Rectangle oldClipRect = clipRect;
  ...
  public int setLineStyle(int style) {
    ...
    switch(style) {
    case Grappa.LINE_SOLID:
    case Grappa.LINE_DASHED:
    case Grappa.LINE_DOTTED:
      lineStyle = style;
      break;
    }
    return oldStyle;
  ...
  public String setLineStyle(String style) {
    ...
    if(style.equals(Grappa.LINE_SOLID_STRING)) {
      lineStyle = Grappa.LINE_SOLID;
    } else if(style.equals(Grappa.LINE_DASHED_STRING)) {
      lineStyle = Grappa.LINE_DASHED;
    } else if(style.equals(Grappa.LINE_DOTTED_STRING)) {
   ...
   *
   * @return the line style as an int.
   */
  ...
  public String getLineStyleString() {
    ...
    switch(lineStyle) {
    case Grappa.LINE_SOLID:
      retString = Grappa.LINE_SOLID_STRING;
      break;
    case Grappa.LINE_DASHED:
      retString = Grappa.LINE_DASHED_STRING;
      break;
    case Grappa.LINE_DOTTED:
      retString = Grappa.LINE_DOTTED_STRING;
      break;
    }
    return retString;
  }
   ...
   * <code>GraphicContext</code> are equal if the values of their 
  ...
  public boolean equals(Object obj) {
    if(!(obj instanceof GraphicContext)) return false;
    GraphicContext testGC = (GraphicContext)obj;
   ...
   *
   * @return a new object which is an instance of GraphicContext
   */
  public Object clone() {
    try {
      GraphicContext gc = (GraphicContext)super.clone();
      gc.setForeground(getForeground());
      gc.setBackground(getBackground());
      gc.setXORColor(getXORColor());
      gc.setFont(getFont());
      gc.setXORMode(getXORMode());
      gc.setClipRect(getClipRect());
      gc.setFillMode(getFillMode());
